...
const cards = [
  {
    ...
    style: "absolute left-0 top-[130px] w-[449px]",
    gap: "gap-9",
  ...
  },
  {
    ...
    description:
      "Lorem ipsum dolor sit amet consectetur. Donec aenean sagittis urna cursus tellus odio. Facilisis quam non at aliquam faucibus hendrerit non.",
    style: "absolute left-[513px] top-[202px] w-[548px]",
    gap: "gap-[109px]",
    textWidth: "w-[449px]",
  ...
  },
  {
    ...
    description:
      "Lorem ipsum dolor sit amet consectetur. Donec aenean sagittis urna cursus tellus odio. Facilisis quam non at aliquam faucibus hendrerit non.",
    style: "absolute left-[1033px] top-[125px] w-[495px] items-center",
    gap: "gap-[34px]",
    textWidth: "w-[441px]",
...
export function ForWhom() {
  return (
    <section className="bg-green-dark px-[100px] py-[80px] max-sm:px-4 sm:px-8 md:px-12 lg:px-16 xl:px-[100px]">
      <div className="relative mx-auto w-[1528px] max-xl:w-full">
        {/* Title row */}
        ...
        {/* Cards container - desktop: absolute positioned, tablet: grid, mobile: stacked */}
        <div className="relative h-[520px] max-xl:h-auto max-xl:mt-8 max-xl:flex max-xl:flex-col max-xl:gap-6">
          {/* Decorative blur ellipse */}
          <div className="absolute left-[31px] top-[38px] h-[392px] w-[392px] rounded-full bg-lilac/40 blur-[500px] max-xl:hidden" />

          {cards.map((card) => (
            <div
              key={card.title + card.width}
              className={`flex flex-col ${card.gap} ${card.mobileGap} ${card.style} max-xl:static max-xl:w-full max-xl:gap-4 max-sm:flex-row max-sm:items-start`}
            >
              <Image
                src={card.image}
                alt={card.title}
                width={card.width}
                height={card.height}
                className={`${card.mobileImg} max-sm:shrink-0 max-sm:object-contain`}
              />
              <div className={`flex flex-col gap-2 ${card.textWidth ?? ""} max-xl:w-full max-sm:flex-1`}>
